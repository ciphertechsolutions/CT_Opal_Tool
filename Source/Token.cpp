...
void AddTokenCellBlock(LPTCGFULLPACKET ComPacket, LPBYTE Table, LPBYTE StartRowUid, int StartRowInt, int EndRow, int StartColumn, int EndColumn)
{
	...
	if(Table != NULL) {
		...
		AddTokenUnsignedInt(ComPacket, 0);
		AddTokenBytes(ComPacket, Table, 8);
...
LPTOKENS ParseResponse(LPTCGFULLPACKET ComPacket)
{
	...
	/* Allocate space for the processed tokens. */
	...
	for(i=0; i<Count; i++) {
		...
		if((*Payload >= 0xA0) && (*Payload <= 0xBF)) {
			...
			} else if((*Payload > 0x80) && (*Payload <= 0x84)) {
				switch(*Payload) {
					case 0x81:
...
/*
 * For a response that contains a byte sequence (Uid, byte table bytes), return
